...
do for [ polariz = 1:1 ] {
   ...
   set xrange[0:1]
	...
	set xtics ("1.5" 0.0, "2.0" 0.50, "2.5" 1.0)
   ...
   set xrange[3.75378527:4.90848581]
	set origin 0.03,  0.28
	# set xtics ("G" 0.0, "M" 0.25, "M" 1.0, "G" 1.70710678, "Z" 2.18986540)
	load 'outputfiles/highsympoints.gnu'
	set xtics ("2.0" 3.75378527, "3.0" 4.33113554, "4.0" 4.90848581)
	# 3 down-down, 4 down-up, 5 up-up, 6 up-down
	set yrange[  ymin : ymax ]
   ...
   set xrange[5.67224843:6.62679388]
	set origin 0.03,  -0.05
	# set xtics ("G" 0.0, "M" 0.25, "M" 1.0, "G" 1.70710678, "Z" 2.18986540)
	load 'outputfiles/highsympoints.gnu'
	set xtics ("0.5" 5.67224843, "1.0" 6.17224843, "1.5" 6.62679388)
	# 3 down-down, 4 down-up, 5 up-up, 6 up-down
	splot matriz1  u 1:2:0:($7+$8+$9+$10) w pm3d notitle \
		, for [i=2:65] fort1 u 1:i:(0.0) w lines notitle lt 6 lc "green" lw 1  \

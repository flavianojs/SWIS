...
spirit_config_file='input_SK.cfg'
spirit_initial_state='spirit_output/spinconfig_SK_initial_random.ovf'

scale_input='inputfiles/pair_SK.txt'
scale_output='inputfiles/pair_SK_temp.txt'
gnuscript=$scripts"plot_dispersion.gnu"
gnuscript=$scripts"disp_unfol_multiPNG4panelsTHIN.gnu"
gnuscript=$scripts"disp_unfol_multi.gnu"
...
#--- Plottings ---------------------------------------------------------
    ...
    if [ $lattice -eq 1 ]; then
        ...
        if [ "$?" -eq "0" ]; then
            if [ $host == 'theospc47' ] || [ $host == 'mpc2976' ] ; then
                :
                # xdg-open $outputpng
            else
                :

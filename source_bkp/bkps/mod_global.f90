...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   subroutine initialization()
      implicit none
      integer :: i, dim
      real(kind=pc) :: basisaux(3), amat(3,3), bmat(3,3), net_magnetization
      complex(kind=pc) :: HP(3,3), HPinv(3,3), aux(3,3)

      ! A list to be read on the input file: inputcardsky.f90
      namelist /input/ dims, npt, nptomega, naucell, ncellpdim, npath, latcons, a1, a2, a3, Jnn, Dnn, kanis, hm0, hmagunitvec, nndist, maxomega, minomega, eta, ncpdim, toprint, unfolding, analytics, calc_occup, mode, polarization
      namelist /input2/ Uweight, syptsMataux, basisname
         
      ! Reading the inputs and allocating variables   
      open(unit=101, file=inputcardname)
      read(101, nml=input)
      !write(*,nml=input)
      ...
      do i = 1, naucell
         read(unit=90, fmt=*) basisaux, anglesphi(i), anglestheta(i), Si(i)
         basis(i,:)=basisaux
      end do
!     anglesphi(:) = 0.d0
!     anglestheta(:) = pi/2.d0
      ...
      PRINT *, "Net Sz=", net_magnetization
   ...
   subroutine kpointsinitialization()
      ...
      do j = 1, npath
         ...
         do i = 1, nkpt_npath(j)
            ...
            kpoints(counter,:) = k
            dkpoints(counter) = sqrt( dot_product(k-prevk,k-prevk) )
            prevk = k
            write(333, fmt=*) k
         end do
      end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   function rotationmatrix(phi,theta)
      !Return the rotation matrix given a angle phi an theta
      implicit none
      ...
      do l1 = 1, naucell
         ...
         do l2 = 1, naucell
            ...
            do j = 1, ncell
               ...
               ! if( modr0j < nndist*1.01 .and. modr0j > 1.d-4 ) then
               if( abs(modr0j - nndist) < zero_toler .and. modr0j > zero_toler ) then
                  nnn=nnn+1
               ...
               Jtemp2 = matmul( Jtemp , HPmatrixj )
               Jtemp = matmul( conjg(transpose(HPmatrixi)) , Jtemp2 )

               if( j==origcell .and. l1==l2 ) then
                  hmagvec(l2,:) = matmul( hmag(:), Rotmatj )
                  hmagvec(l2,:) = matmul( hmagvec(l2,:), HPmatrixj )
               end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   subroutine printinginput()
      ...
      print *
      print *, "High symmetry point path location:"
...
!$          call OMP_unset_lock(lck)
   ...
   function delta(omega)
      complex(kind=pc), intent(in) :: omega
      real(kind=pc) :: delta
   ...
   subroutine outputdata(disp_matrix,spectra,rightevector,evalues)
      ...
      write(formt,fmt="(a,i0,a)") "(", naucell+1, "f16.8)"
      ...
      do i = 1, effnkpt
         ...
         write( unit=90, fmt=formt ) path/units, disp_matrix(i,:)
         ...
         do mode = 1, naucell
            n = naucell+1 - mode
            write( unit=123, fmt="(2i6, 4f14.8,' ')", advance="no" ) i, mode, real(evalues(i,n)), k
